...
export default function CreatorContractDetailPage() {
        ...
        <FadeIn>
          <Card>
            <CardContent className="py-16 text-center">
              ...
              <Button className="mt-4" onClick={() => router.push('/creator/contracts')}>
                목록으로 돌아가기
              </Button>
            </CardContent>
          </Card>
  ...
  const canSign = contract.status === 'pending_sign' && !contract.signed_by_creator;
  const canSubmitContent = contract.status === 'active' || contract.status === 'revision_requested';

  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <FadeIn>
        <button
          onClick={() => router.push('/creator/contracts')}
        ...
        <FadeIn delay={0.3}>
          <Card>
            <CardContent className="p-5 flex flex-col sm:flex-row gap-3">
              ...
              {canSubmitContent && (
                <Button
                  onClick={() => handleAction('submit-content')}
                  disabled={actionLoading === 'submit-content'}
                  className="flex-1"
                >
                  {actionLoading === 'submit-content' ? (
